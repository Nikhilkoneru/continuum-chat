# Copilot Instructions for continuum-chat
...
2. **Build daemon**: `cargo build --manifest-path apps/daemon/Cargo.toml --bin continuum`
3. **Build client**: `node apps/client/scripts/build.mjs` — this regenerates `dist/`, `service-worker.js`, and the build version hash
4. **Restart backend**: Kill the old process on port 4000, then start `HOST=0.0.0.0 apps/daemon/target/debug/continuum daemon run`
...
- **Backend**: `continuum` is the single product CLI. It runs the Rust + Axum daemon and manages auto-start service installation.
...
- Dev run: `cargo run --manifest-path apps/daemon/Cargo.toml --bin continuum -- daemon run`
- Port override: `continuum daemon run --port 4310`
- Health + diagnostics: `continuum daemon doctor`, `continuum daemon paths`
- Start at login: `continuum daemon service install`
- Status/restart: `continuum daemon service status`, `continuum daemon service restart`
- Update the installed CLI: `continuum update`
...
- Build version injected into `index.html` as `window.__CONTINUUM_BUILD_VERSION__`
...
- **Runtime control surface**: The browser Settings modal shows daemon version, lifecycle mode, log/config paths, and the exact `continuum` commands for restart/update/opening the UI
