...
pub async fn create_thread(
    ...
    let project_names = if let Some(project_id) = thread.project_id.clone() {
        load_project_names(db, [project_id]).await?
    } else {
        HashMap::new()
    };
    Ok(Some(into_summary(thread, &project_names)))
}
...
pub async fn list_threads(
    ...
    Ok(threads
        .into_iter()
        .map(|thread| into_summary(thread, &project_names))
        .collect())
}
...
pub async fn get_thread(
    ...
    let project_name = load_project_name(db, thread.project_id.as_deref()).await?;
...
pub async fn update_thread(
    ...
    if let Some(project_id) = actual_project_id {
        if let Some(project_name) = load_project_name(db, Some(&project_id)).await? {
...
mod tests {
    ...
    async fn creates_thread_for_project_and_truncates_preview() {
        ...
        update_thread_preview(&db, &thread.id, &format!("{}\n{}", "hello", "word ".repeat(80)))
            .await
            .unwrap();
        let updated = get_thread(&db, &config.daemon_owner_id, &thread.id)
            .await
            .unwrap()
